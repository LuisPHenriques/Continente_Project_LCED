...
names(mydata1)<-c("Customer", "precision_Baseline", "precision_RandomForest", "FamilyMembers", "Lifestage")
attach(mydata1)
...
library(ez)
model<-ezANOVA(data=mydata1_long, dv=.(precision), wid=.(Customer), within=.(Model), between=.(FamilyMembers, Lifestage), type=3, detailed=TRUE, return_aov=FALSE)
model
...
Y<-cbind(precision_Baseline, precision_RandomForest)
...
# for Model=Baseline
model2_1<- lm(precision_Baseline ~ FamilyMembers + Lifestage + FamilyMembers:Lifestage, data=mydata1, contrasts = list(FamilyMembers=contr.sum, Lifestage=contr.sum))
model2_1_Anova_III <- Anova(model2_1, type=3)
model2_1_Anova_III

# for Model = RandomForest
model2_2<- lm(precision_RandomForest ~ FamilyMembers + Lifestage + FamilyMembers:Lifestage, data=mydata1, contrasts = list(FamilyMembers=contr.sum, Lifestage=contr.sum))
